...
database=pyodbc.connect(r"Driver={SQL Server};Server=MYLAPTOP;Database=database_SQLServer;uid=Amoozesh;pwd=123")
cursor=database.cursor()


def create_car_table():
    cursor.execute("CREATE TABLE car_table(car_id INTEGER PRIMARY KEY, maker TEXT, model TEXT, year INTEGER, price FLOAT, color TEXT)")
    database.commit()
